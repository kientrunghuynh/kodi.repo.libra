...
class serversList:
    ...
    def mode(self, domain):
        try:
            m = int(''.join(i[1] for i in self.servers if i[0] == domain))
        except:
            m = 0
        return m
    ...
    def move(self, server, step):  # sl:server location, ol: ordinal location, step:up=-1,down=+1
        ...
        temp = self.ordinal[ol + step];
        self.ordinal[ol + step] = sl;
        self.ordinal[ol] = temp
        xrw('free_servers.dat', ','.join(str(i) for i in self.ordinal))

    def moveDown(self, server):
        sl = self.servers.index([i for i in self.servers if i[0] == server][0])
        ol = self.ordinal.index([i for i in self.ordinal if i == sl][0])
        temp = self.ordinal[ol + 1];
        self.ordinal[ol + 1] = sl;
        self.ordinal[ol] = temp
        xrw('free_servers.dat', ','.join(str(i) for i in self.ordinal))
    ...
    def search(self, string):
        ...
        try:
            j = json.loads(xsearch('\((\{.+?\})\)', xread(href)))
        except:
            j = {}
        if not j.get('results', {}):
            mess(u'Tìm gần đúng')
            try:
                j = json.loads(xsearch('\((\{.+?\})\)', xread(url.replace('%22', ''))))
            except:
                j = {}
            if not j: return []
        ...
        def detail(l):
            ...
            try:
                img = l['richSnippet']['cseImage']['src'].encode('utf-8')
            except:
                img = ''
...
class googlesearch:
    ...
    def content(self, cx, start, string):
        ...
        try:
            d = json.loads(xsearch('\((\{.+?\})\)', xread(self.url % (cx, start, '%22' + string + '%22'))))
        except:
            d = {}
        if not d.get('results', {}):
            mess(u'Tìm gần đúng ...', 'xshare')
            try:
                d = json.loads(xsearch('\((\{.+?\})\)', xread(self.url % (cx, start, string))))
            except:
                d = {}
...
class fshare:  # https://www.fshare.vn/home/Mục chia sẻ của thaitni/abc?pageIndex=1
    ...
    def __init__(self, username='', password='', id=''):
        ...
        if self.myAcc:
            ...
            if self.logged == 'success':
                ...
                self.myFavourite_id = ''.join(
                    xsearch('(\w{10,})', s[1]) for s in myfolder if s[0] == 'xshare_favourite')
                if not self.myFavourite_id:
                    self.myFavourite_id = self.add_folder(folder_name='xshare_favourite', in_dir_id='0')

    def fetch(self, url, data=None):
        try:
            response = urlfetch.fetch(url, headers=self.hd, data=data)
        except:
            response = None
        return response
    ...
    def logout(self):
        if self.logged:
            ...
            if response and response.status == 302:
                ...
                mess(u'Logout không thành công!', 'Fshare.vn')
    ...
    def get_maxlink_free(self, url):
        ...
        try:
            d = eval(xread('http://pastebin.com/raw/JkrunuTH'))
        except:
            d = []
        link = '';
        loop = 0;
    ...
    def get_folder(self, url):
        ...
        b = xread(url, self.hd)
        if xsearch('(Số lượng: 0)', b):
            mess('Folder %s is empty' % url, 'Fshare.vn');
            return result
        elif not b:
            mess('Folder %s find not found' % url, 'Fshare.vn');
            return result
        ...
        for s in list:
            ...
            if type == 'file':
                link = 'https://www.fshare.vn/file/%s' % id
            elif xsearch('(\A[0-9A-Z_]{10,20})', title):  # MyFshare item name
                ID = xsearch('(\A[0-9A-Z_]{10,20})', title)
                title = title.split(' ', 1)[-1].strip()
                if 'FOLDER' in ID:
                    link = 'https://www.fshare.vn/folder/%s' % ID.replace('FOLDER', '')
                elif 'FILE' in ID:
                    link = 'https://www.fshare.vn/file/%s' % ID.replace('FILE', '')
                elif xget('https://www.fshare.vn/folder/%s' % ID):
    ...
    def myFshare_add(self, url, name):
        ...
        if response and response.status == 200:
            mess(u'Add a item to MyFshare success', 'Fshare.vn')
        else:
            mess(u'Add a item to MyFshare fail!', 'Fshare.vn')
    ...
    def Favorite_add(self, url):
        ...
        if response and response.status == 200:
            result = True;
            mess(u'Add a item to MyFshare Favorite success', 'Fshare.vn')
        else:
            result = False;
            mess(u'Add a item to MyFshare Favorite fail!', 'Fshare.vn')
        return result
    ...
    def Favorite_remove(self, name):
        ...
        if response and response.status == 200:
            result = True;
            mess(u'Remove a item from MyFshare Favorite success', 'Fshare.vn')
        else:
            result = False;
            mess(u'Remove a item from MyFshare Favorite fail!', 'Fshare.vn')
        return result
    ...
    def add_folder(self, folder_name, in_dir_id='0'):
        ...
        if result_code == 200:
            ...
            mess(u'Add folder fail !', 'Fshare.vn');
            result = ''
    ...
    def get_myFolder(self, folder_name='', page=0):
        ...
        if page > 0 and folder_name == 'xshare_favourite':
            ...
            url = 'https://www.fshare.vn/home/' + folder_name + '?_pjax=%23pjax_content'  # ;print url,self.hd
    ...
    def upload_file(self, fn):  # Chua xong
        ...
        data = '{"SESSID":"%s","name":"%s","path":"%s","secured":"1","size":"%d","token":"%s"}' % (
            session_id, name, path, size, self.get_token())
        response = self.fetch('https://www.fshare.vn/api/session/upload', data)

        if response and response.status == 200:
            try:
                response = urlfetch.fetch(response.json['location'].replace('http:', 'https:'), headers=self.hd,
                                          data=data)
            except:
                response = None
            if response and response.status == 200:
                result = True;
                mess(u'Upload file to MyFshare success', 'Fshare.vn')
    ...
    def myFavourites_add(self, s):
        result = []
        if not self.myFavourite_id:
            mess('Your Fshare Favourite ID not found!')
        else:
            import time
            loop = True;
            ...
            while loop:
                ...
                if i:
                    ...
                    loop = False  # Chua xu ly delete neu that bai
    ...
    def myFavourites_loads(self, page):
        ...
        for i in sorted(list(set([i[0] for i in s])), reverse=True):
            try:
                m = urllib2.base64.urlsafe_b64decode(''.join(j[2] for j in s if j[0] == i))
            except:
                m = ''
    ...
    def searchFollow(self, q):
        ...
        for s in refas('(<li.+?/li>)', b):
            ...
            if t:
                t = int(t);
                title = '%s [COLOR cyan]%d (follow)[/COLOR]' % (title, t)
            else:
                t = 0
        ...
        def abc(s):
            return not re.search('photoshop|.hần .ềm', s)
...
class fptPlay:  # from resources.lib.servers import fptPlay;fpt=fptPlay(c)
    ...
    def login(self):
        phone_fptplay = get_setting('phone_fptplay');
        password = get_setting('pass_fptplay')
        ...
        url = 'https://fptplay.net/user/login'
        req = urllib2.Request('https://fptplay.net/user/login', data)
        try:
            b = urllib2.urlopen(req, timeout=30)
        except:
            pass
        cookie = xcookie(cookie)
        if 'laravel_id' in cookie:
            mess(u'Login thành công', 'fptplay.net');
    ...
    def liveLink(self, url):
        ...
        try:
            link = json.loads(b).get('stream') + '|User-Agent=Mozilla/5.0 AppleWebKit/537.36'
        except:
            link = ''
        return link
    ...
    def playLink(self, url):
        def stream(href, id, epi='1'):
            ...
            try:
                link = json.loads(xread(href, self.hd, data)).get('stream') + HD
            except:
                b = xread(url);
                id = xsearch("var id = '(.+?)'", b)
                data = ec({'id': id, 'type': 'newchannel', 'quality': '3', 'episode': epi, 'mobile': 'web'})
...
class hayhayvn:
    ...
    def getLink(self, url):
        ...
        if tap: tap = '_%s_' % tap
        if '/show-' in url:
            url = 'http://www.hayhaytv.vn/getsourceshow/%s' % (xsearch('-(\w+)\.html', url) + tap)
        else:
    ...
    def getLink1(self, url):
        b = xread(url)
        link = xsearch('file.{,5}"(.+?)"', b);
        sub = ''
        if link:
            sub = xsearch("var track.{,5}'(.+?)'", b)
        else:
            url = 'http://www.hayhaytv.vn/getsource/%s' % xsearch("FILM_KEY = '(.+?)'", b)
            b = xread(url, self.hd)
            try:
                links = [(i.get('file').replace('\\', ''), i.get('label')) for i in eval(b)]
            except:
                links = []
...
class phim3s_net:
    def __init__(self, username, password):
        self.session = urlfetch.Session(
            headers={'User_Agent': 'Mozilla/5.0 (Android 4.4; Mobile; rv:42.0) Gecko/41.0 Firefox/42.0',
                     'X-Requested-With': 'XMLHttpRequest', 'Cookie': ''})
        self.login(username, password)

    def fetch(self, url, data=''):
        try:
            response = self.session.fetch(url, data=data)
        except:
            response = None
        return response
...
class kPhim:
    ...
    def detail(self, l):
        ...
        for s in l:
            ...
            if t:
                title += ' - ' + t.group(1) + ' ' + t.group(2)
            elif xsearch('>(.+?)</span>', s):
                title += ' - ' + xsearch('>(.+?)</span>', s)
            items.append((title, href, img))
        return items
    ...
    def getLink(self, url):
        ...
        for href, label in ls([(i[0], rsl(i[1])) for i in j]):
            try:
                link = urllib2.urlopen(href).geturl();
                break
            except:
                pass
        return link
...
class hdVietnamn:  # from resources.lib.servers import hdvn;hdvn=hdvn()
    ...
    def login(self):
        ...
        if addon.getSetting('usernameh') in content:
            ...
            mess(u'Login không thành công!', 'hdvietnam.com')
    ...
    def forums(self, url):
        ...
        def detail(s):
            ...
            if not timestring:
                ...
                datestring = '[COLOR gold]%s %s[/COLOR] ' % (xsearch('data-datestring="(.+?)"', s), timestring)
    ...
    def getlink(self, url, buian=''):
        ...
        def s1(s):
            return [j for j in srv if j in s.lower()]
        ...
        def s2(string, href, c):  # get title
            if buian:
                ...
                while len(s) < 10 and (p - i) > 0:
                    i += 100;
                    s = xsearch(pattern, c[p - i:p]).strip()
        ...
        content = '\n=*=\n'.join(' '.join(i.split()) for i in s);
        s = ''
        temp = re.findall('<a href="([^"]+?)" target="_blank">(.+?)</a>', content)
        temp1 = [i for i in temp if s1(i[0])];
        n = [i[0] for i in temp1]
        temp = [i for i in re.findall('>(http[^<]+?)<', content) if '...' not in i and s1(i)]
        temp1 += [(i, '') for i in temp if i not in n];
        n = [];
        temp = []
    ...
    def removeRoom(self, url):
        ...
        if href:
            ...
            result = False;
            mess('Link subscription remove not found!', 'HDVietnam.com')
        return result
...
class youtube:  # https://www.youtube.com/watch?v=rZftpdEKzeY
    ...
    def getNextPageURL(self, url, j):
        return re.sub('&pageToken=.*', '', url) + '&pageToken=' + j.get('nextPageToken') if j.has_key(
            'nextPageToken') else ''
    ...
    def getItems(self, url, urlNextPage):
        ...
        for i in j.get('items', []):
            ...
            if not title or not id or 'sex' in title:
                continue
            else:
                items.append((title, id, img, kind, publishedAt))
    ...
    def getElements(self, id, kind):
        ...
        if kind == 'guideCategoryListResponse':
            ...
            url = url % (
                'channels', 'part=snippet%2Clocalizations%2CcontentDetails&categoryId=' + id) if not href else href
        elif kind == 'playlists':
            ...
            j = self.ytRead(href)
            urlNextPage = self.getNextPageURL(href, j)
            ids = ','.join(i.get('snippet', {}).get('resourceId', {}).get('videoId', '').encode('utf-8') for i in
                           j.get('items', []))
            ...
            if not href: href = url % (
                'search', 'type=video&part=snippet&q=' + urllib.quote_plus(id))  # id:stringsearch
            j = self.ytRead(href)
            urlNextPage = self.getNextPageURL(href, j)
...
class imovies:
    ...
    def maxLink(self, url):
        ...
        if not link and 'Phim này chưa được phát hành trên IMovies' in b:
            ...
            mess('File invalid or deleted!', 'IMovies.vn')
        return link, sub
    ...
    def htmlPage(self, url, mode=0):
        b = xread(url);
        items = []
        if 'Ooops.Đã Có lỗi xảy ra!' in b:
            mess(u'Ooops.Đã Có lỗi xảy ra!', 'imovies.vn')
        elif mode == 1:
    ...
    def ajaxPage(self, url, sort=''):
        ...
        def geti(i, s):
            return i.get(s, '').encode('utf-8') if i.get(s, '') else ''

        keys = [('Daily', 'hôm nay'), ('Weekly', 'trong tuần'), ('Monthly', 'tháng')] if sort == 'Hit' else [
            ('Data', '')]
        items = []
        for key in keys:
            if len(keys) > 1:
            ...
            for i in d:
                title = '%s %s' % (geti(i, 'Title'), geti(i, 'Name'));
                dir = False
                if i.get('Year'): title += ' [COLOR orangered]%s[/COLOR]' % i.get('Year')
                if geti(i, 'State'):
                    title = namecolor(title, self.c) + ' [COLOR cyan]%s[/COLOR]' % geti(i, 'State');
                    dir = True
...
class iMax:  # ;f=open(r'd:\xoa1.html','w');f.write(b);f.close()
    ...
    def thread(self, url):
        ...
        for s in re.findall('(<tr.+?/tr>)', s, re.S):
            ...
            mem = xsearch('>([^<]+?)</span>', s)
            if mem:
                title = '[COLOR gold]%s[/COLOR] [COLOR blue]%s[/COLOR] ' % (label.group(1).strip(), mem) + title
            else:
                title = '[COLOR gold]%s[/COLOR] ' % label.group(1).strip() + title
    ...
    def search(self, url):
        ...
        if not j.get('results', {}):
            ...
            try:
                j = json.loads(xsearch('\((\{.+?\})\)', xread(url)))
            except:
                j = {}
            if not j: return []
        l = []
        for i in j.get('results', {}):
            ...
            p = xsearch('p=(\d+)', href)
            if t:
                href = 'http://i-max.vn/forum/showthread.php?t=' + t;
                q = 'thread'
            elif f:
                href = 'http://i-max.vn/forum/forumdisplay.php?f=' + f;
                q = 'category'
            elif p:
                href = 'http://i-max.vn/forum/showthread.php?p=' + p;
                q = 'thread'
            else:
                print '------------------' + href;
                continue
            try:
                img = i['richSnippet']['cseImage']['src']
            except:
                img = ''
...
class taiphim:
    ...
    def login(self):
        data = urllib2.base64.b64decode(
            'bG9naW49eHNoYXJlJTQwdGhhbmh0aGFpLm5ldCZyZWdpc3Rlcj0wJnBhc3N3b3JkPXhzaGFyZSZjb29raWVfY2hlY2s9MSZyZWRpcmVjdD0lMkYmX3hmVG9rZW49');
        url = 'http://taiphimhd.com/login/login';
        hd = {'User-Agent': 'Mozilla/5.0'}
        try:
            a = self.fetch(url, hd);
            hd['Cookie'] = a.cookiestring;
            a = self.fetch(url, hd, data);
            self.hd['Cookie'] = a.cookiestring
            if a.status == 303:
    ...
    def fetch(self, url, headers=None, data=None):
        ...
        try:
            response = urlfetch.fetch(url, headers=headers, data=data)
        except:
            response = None
    ...
    def search(self, s):
        ...
        try:
            j = json.loads(xsearch('\((\{.+?\})\)', a.body)).get('results')
        except:
            j = []
        if not j: return []
        l = []
    ...
    def search1(self, s):
        ...
        try:
            b = self.fetch(b.headers.get('location')).body
        except:
            b = ''
...
class sieunhanh:
    ...
    def page(self, url):
        content = xread(url);
        items = []
        for s in [i for i in content.split('<div class="block-base movie">') if 'data-id' in i]: items.append(
            self.item(s))
        ...
        for s in ('director', 'phim', 'actor', 'show'):
            for i in j.get(s, []):
                ...
                if not eps:
                    ...
                    title = '%s [COLOR blue]%s[/COLOR]' % (namecolor(title, self.color), eps);
                    dir = True
...
class chiaseNhac:
    def __init__(self, username, password):
        self.session = urlfetch.Session(
            headers={'User_Agent': 'Mozilla/5.0 (Android 4.4; Mobile; rv:42.0) Gecko/41.0 Firefox/42.0',
                     'Cookie': 'vq=i%3A1080%3B; mq=i%3A500%3B'})
...
class nhaccuatui:
    def __init__(self):
        ...
        if filetime('nhaccuatui.cookie') > 10:
            self.login()
        else:
    ...
    def login(self):
        ...
        opener = urllib2.build_opener(cookie)
        urllib2.install_opener(opener)
        username = get_setting('nct_u');
        password = get_setting('nct_p')
    ...
    def cleanTag(self, s):
        return re.sub('\!|\[|\]|<|>|CDATA', '', s).strip()

    def getData(self, s, key):
        data = ''
        if key == 'link':
            keys = ['highestquality', 'highquality', 'lowquality', 'locationHQ', 'location']
        else:
            keys = [key]
    ...
    def search(self, s, url):
        ...
        if s != 'search-in':
            try:
                j = json.loads(xread(self.speedsearch + s)).get('data', dict())
            except:
                j = {}
            ...
            for k in [j.get(i, dict()) for i in ['singer', 'video', 'playlist', 'song']]:
                for m in k:
                    ...
                    singer = ','.join(
                        m.get('singer', [])[n]['name'].encode('utf-8') for n in range(len(m.get('singer', []))))
    ...
    def getCounter_sg(self, s):
        ...
        try:
            counter = json.loads(xread(href)).get('data')['songs']
        except:
            counter = {}
        return counter
    ...
    def getCounter_pl(self, s):
        ...
        try:
            counter = json.loads(xread(href)).get('data')['playlists']
        except:
            counter = {}
        return counter
...
class vietsubhd:
    ...
    def getLink(self, url):
        ...
        try:
            ...
            j = {}
        if j.get('link') and isinstance(j.get('link'), unicode):
            link = j.get('link')
        elif j.get('link') and isinstance(j.get('link'), list):
            link = googleItems(j.get('link'))
...
class hdonline:
    ...
    def getDetail(self, s):
        ...
        if not title: title = xsearch('<p class="name-en">(.+?)</p>', s) + ' ' + xsearch('<p class="name-vi">(.+?)</p>',
                                                                                         s)
        ...
        if eps:
            title = namecolor(title + ' - [COLOR gold]%s[/COLOR]' % eps, self.c);
            q = 'eps';
            dir = True
        else:
            href = href.replace('m.hdonline', 'hdonline');
            q = 'play';
            dir = False
    ...
    def maxLink(self, url):
        ...
        try:
            ...
            if j.get('level'):
                items = ls([(i.get('file'), rsl(i.get('label'))) for i in j.get('level')])
            elif j.get('file'):
                items = j.get('file')
    ...
    def getLink(self, url, ep='1'):
        ...
        try:
            j = json.loads(xread(url, self.hd))
        except:
            j = {}
        items = [];
        sub = ''
...
class phimBatHu:
    ...
    def getDirectLink(self, url):
        def getLink(s, key):  # chonserver values="Thuyết minh|VietSub|Không"
            try:
                j = json.loads(xsearch('"sources":(\[[^\]]+?\])', s))
            except:
                j = {}
            for i in j:
                try:
                    i['file'] = urllib.unquote(gibberishAES(i.get('file', ''), key))
                except:
                    pass
            link = googleItems(j, 'file', 'label')
            return link
        ...
        if not servers or chonserver == 'Không':
            ...
            for s in servers:
                title = xsearch('>([^<]+?)</a>', s).upper();
                found = False
                if 'T MINH' not in title and 'VIETSUB' not in title:
                    found = True;
                    break
                elif chonserver == 'Thuyết minh' and 'T MINH' in title:
                    found = True;
                    break
                elif chonserver == 'VietSub' and 'VIETSUB' in title:
                    found = True;
                    break
        ...
        if not link:
            ...
            for s in strings:
                ...
                b = xread(href)
                try:
                    link = googleItems(json.loads(b), 'file', 'label')
                except:
                    pass
                if link: break
        ...
        if link:
            try:
                mess(u'Play on: ' + playOn.decode('utf-8'))
            except:
                pass
        else:
...
class mphim:
    ...
    def maxLink(self, url):
        ...
        if items:
            items = [(href.replace('&amp;', '&'), label) for href, label in items]
        else:
...
class phim14com:
    ...
    def getLink(self, url):
        ...
        if id:
            b = xread('http://phim14.net/ajax/episode.html', data='do=load_episode&episodeid=%s' % id)
        else:
            b = xread(url)
        href = xsearch('link:"(.+?)"', b)
    ...
    def getDaily(self, url):
        ...
        linkYT = xsearch('src="(.+?)"', s).replace('&amp;', '&')
        data = 'url=' + xsearch('src="(.+?)["|\?]', s);
        print data
        b = xread('http://player8.phim14.net/3008/plugins/plugins_player.php', data=data)
        try:
            j = json.loads(xsearch('"qualities":(\{.+?\]\}),', b, 1, re.S))
        except:
            j = {}

        def getURL(l):
            return ''.join(i.get('url', '') for i in l if 'mpegURL' in i.get('type'))
...
class vtvgovn:
    ...
    def liveGoList(self, url):
        def e(i):
            return i.get('name').encode('utf-8'), i.get('id').encode('utf-8'), i.get('thumbnailUrl').encode('utf-8')
    ...
    def vodList(self, url):
        def e(i):
            return i.get('name').encode('utf-8'), i.get('downloadUrl').encode('utf-8'), i.get('thumbnailUrl').encode(
                'utf-8')
    ...
    def live(self, id):
        ...
        try:
            link = json.loads(xread(url, self.hd)).get('data', '')
        except:
            link = ''
        print link
        return link
    ...
    def golive(self, url, l='04'):
        ...
        try:
            url = json.loads(url).get('flavors')[0].get('url')
        except:
            url = ''
    ...
    def golive_f4m(self, url='1_rhex2pfs'):
        url = json.loads(xread(
            'http://cdnapi.kaltura.com/p/2111921/sp/211192100/playManifest/entryId/%s/format/hdnetworkmanifest/protocol/http/uiConfId/35084022/?referrer=aHR0cDovL3Z0dmdvLnZu&responseFormat=json' % url)).get(
            'flavors')[0]['url']
...
class phimMedia:
    def maxLink(self, url):
        ...
        try:
            items = ls([(i.get('file', ''), rsl(i.get('label', ''))) for i in eval(s)])
        except:
            items = []
        return items
...
class k88com:
    ...
    def episode(self, url):
        b = xread(url.replace('/phim/', '/xem-phim-online/'));
        d = {};
        count = 0
    ...
    def openload(self, url):
        ...
        if result:
            ...
            try:
                ...
                vid = False;
                mess(u'Cài đặt module youtube.dl để get link phim này')
            if vid:
        ...
        if ticket:
            ...
            try:
                ...
                mess('Get link from openload.co Failed')
        return link
    ...
    def getLink(self, url):
        ...
        if len(href) > 20:
            data = urllib.urlencode({'link': href});
            label = 0  # ;mess('link')
            ...
            try:
                ...
                if isinstance(j, unicode):
                    ...
                    items = ls([(i.get('link', ''), rsl(i.get('label', ''))) for i in j])
                for href, r in items:
...
class fcinenet:
    ...
    def login(self):
        ...
        pw = addon.getSetting('fcine_pass')
        ...
        try:
            cookie = urllib2.urlopen(urllib2.Request(url, '', headers)).read()
        except:
            cookie = ''
    ...
    def lisItem(self, s):
        ...
        for s in S:
            ...
            if not title or not href: continue
            if 'HDRip.png' in s:
                title += ' [COLOR orange]HDRip[/COLOR]'
            elif 'Bluray.png' in s:
                title += ' [COLOR orange]Bluray[/COLOR]'
            elif 'WEBrip.png' in s:
                title += ' [COLOR orange]WEBrip[/COLOR]'
            elif 'WEB-DL.png' in s:
                title += ' [COLOR orange]WEB-DL[/COLOR]'
            img = xsearch('src="(.+?)"', s)
            items.append((title, href, img))
        return items
    ...
    def getFshare(self, url):
        ...
        def check(s):
            return '<strong>Phụ đề:</strong>' in s or '<strong>Tải phim:</strong>' in s
...
class taiphimhdnet:
    ...
    def login(self):
        name = addon.getSetting('taiphimhdnet_user');
        pw = addon.getSetting('taiphimhdnet_pass')
        ...
        if 'DRUPAL_UID=' in cookie:
            ...
            cookie = '';
            mess(u'Login không thành công!', 'taiphimhd.net')
        return cookie
    ...
    def xemnhieu(self, l, d, p):
        ...
        b = xread(url)
        try:
            s = eval(re.sub('"status".+?,', '', b)).get('display')
        except:
            s = ''
    ...
    def download(self, url):
        try:
            res = urllib2.urlopen(urllib2.Request(url, headers=self.hd))
        except:
            res = ''
        if res:
            b = res.read(500 * 1024)
...
class vnZoom:
    ...
    def login(self):
        ...
        try:
            ...
            cookie = xsearch('(.+?);', res.headers.get('set-cookie'));
            mess('Login OK')
        except:
            cookie = '';
            mess('Login Failed')
    ...
    def bb_forumpwd(self, blockrow):
        ...
        data = {'do': 'doenterpwd', 'url': u, 'forumid': fm, 'forceredirect': f, 'securitytoken': t, 's': s,
                'newforumpwd': n}
        ...
        opener.addheaders = self.hd.items()
        try:
            opener.open(href, urllib.urlencode(data))
        except:
            pass
